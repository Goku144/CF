...
cf_status cf_bytes_slice(cf_bytes src, cf_usize offset, cf_usize len, cf_bytes *dst)
{
  ...
  if(!cf_bytes_is_valid(src)) return CF_ERR_STATE;
...
{
  if(dst == CF_NULL) return CF_ERR_NULL;
  if(!cf_bytes_mut_is_valid(src)) return CF_ERR_STATE;
  if(offset > src.len || len > src.len - offset) return CF_ERR_BOUNDS;
  dst->data = src.data + offset;
  dst->len = len;
  return CF_OK;
...
cf_status cf_buffer_reserve(cf_buffer *buffer, cf_usize min_cap)
{
  ...
  if(buffer->cap >= min_cap) return CF_OK;
  cf_u8 *tmp;
  if(buffer->data == CF_NULL) tmp = malloc(min_cap * sizeof (cf_u8));
  else tmp = realloc(buffer->data, min_cap * sizeof (cf_u8));
...
cf_status cf_buffer_append_byte(cf_buffer *buffer, cf_u8 byte)
{
  ...
  if(buffer->cap == buffer->len) 
  {
    ...
    buffer, 
    cf_next_cap(buffer->cap, buffer->len + 1)
    )) != CF_OK) return state;
  }
...
cf_status cf_buffer_append_bytes(cf_buffer *buffer, cf_bytes bytes)
{
  ...
  if(buffer->cap < (buffer->len + bytes.len))
  {
    ...
    if((state = cf_buffer_reserve(
      buffer, 
      cf_next_cap(buffer->cap, buffer->len + bytes.len)
    )) != CF_OK) return state;
  }
