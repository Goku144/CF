...
/**
 * @brief Initialize an array with optional capacity for array elements.
 *
 ...
 * @return `CF_OK` on success or `CF_ERR_OOM` when the initial allocation
 * fails.
 */
cf_status cf_array_init(cf_array *array, cf_usize capacity);

/**
 * @brief Ensure that an array can hold at least the requested element count.
 *
 ...
 * @param capacity Minimum capacity required after the call, measured in
 * elements.
...
/**
 * @brief Release an array's owned storage and reset it to an empty state.
 *
...
/**
 * @brief Mark an array as empty without releasing its allocated storage.
 *
...
/**
 * @brief Append one or more elements to the end of an array.
 *
 ...
 * @param element Pointer to the first array element to append.
 ...
 *
 * @param array Array to inspect.
 * @return `CF_TRUE` when the array length is zero, otherwise `CF_FALSE`.
 */
